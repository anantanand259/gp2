...
export default {
    async fetch(request, env, ctx) {
        ...
        try {
            ...
            } else if (path.startsWith('/api/rag/')) {
                response = await handleRAGProxy(request, env);
            } else {
...
// ─── /api/rag/* → Proxy to Python Backend ───
async function handleRAGProxy(request, env) {
    const backendUrl = env.RAG_BACKEND_URL;
    if (!backendUrl) {
        return jsonResponse({ 
            error: 'RAG Backend not configured. Set RAG_BACKEND_URL secret in Wrangler.',
            tip: 'wrangler secret put RAG_BACKEND_URL'
        }, 500);
    }
