...
const CHATBOT_CONFIG = {
    // ┌──────────────────────────────────────────────────────┐
    // │  🌐  API PROXY — All requests go through this       │
...
class ChatbotUI {
    ...
    // ─── Multi-LLM Call: Proxy (Primary) → Gemini via Proxy (Fallback) ───
    async callLLM(prompt) {
        // Build conversation history for context
        const recentHistory = this.conversationHistory.slice(-CHATBOT_CONFIG.MAX_HISTORY);

        // ── STEP 1: Try chat via proxy (handles API keys server-side) ──
        try {
            console.log('[Chatbot] Calling proxy for chat (Qwen 2.5 14B via OpenRouter)...');
            ...
            console.warn('[Chatbot] Proxy chat failed:', err.message);

            // ── STEP 2: Fallback — try Gemini models via proxy ──
            console.log('[Chatbot] Trying Gemini fallback models via proxy...');
            const geminiBody = this._buildGeminiBody(prompt, recentHistory);

            let lastError = err;
            for (const model of CHATBOT_CONFIG.GEMINI_MODELS) {
                try {
                    console.log(`[Chatbot] Trying Gemini model via proxy: ${model}...`);
                    ...
                    console.warn(`[Chatbot] Gemini ${model} failed:`, geminiErr.message);
                }
            }
    ...
    async _callOpenRouter(prompt, history) {
        ...
        // ✅ Call YOUR Worker proxy, not OpenRouter directly
        const response = await fetch(`${CHATBOT_CONFIG.API_PROXY_URL}/api/chat`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ messages, model: CHATBOT_CONFIG.OPENROUTER_MODEL })
        });
        ...
        // Worker returns Gemini-style response format
        const text = data?.candidates?.[0]?.content?.parts?.[0]?.text;
        if (!text) throw new Error('Empty response from proxy');
        return text;
    }

    // ─── Gemini Fallback via Proxy ───
