...
const CHATBOT_CONFIG = {
    ...
    // Gemini models: tried in order via proxy
...
- If NO context is relevant, or if the user asks a general question (math, science, general knowledge), YOU MUST POLITELY DECLINE. Say "This specific information is not available in our knowledge base. Please contact the college directly or visit gpa.ac.in."
- DO NOT use outside knowledge or internet surfing.
...
class ChatbotUI {
    ...
    // ─── Process Query (RAG Backend + LLM) ───
    async processQuery(query) {
        const lowerQuery = query.toLowerCase();
        const isCollegeQuery = COLLEGE_KEYWORDS.some(kw => lowerQuery.includes(kw));

        // ── STEP 1: Try RAG Backend for college-related queries ──
        if (isCollegeQuery && this.ragAvailable) {
            try {
        ...
        // ── STEP 2: Direct LLM call via proxy ──
        let prompt = '';
        if (isCollegeQuery) {
            prompt = `USER QUESTION: ${query}\n\nAnswer this question about Government Polytechnic Adityapur. Use the college facts in your system prompt. If you don't have specific data, suggest checking the official website gpa.ac.in or contacting the college.`;
        } else {
