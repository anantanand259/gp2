/* ═══════════════════════════════════════════════════════════════
   ...
   Client-side RAG · OpenRouter (Qwen) + Gemini Fallback
   ═══════════════════════════════════════════════════════════════ */
...
const CHATBOT_CONFIG = {
    ...
    // This powers the primary Qwen model (free tier)


    // Gemini API Key — Used as fallback & for embeddings
    ...
    // Embedding model (Gemini only — OpenRouter doesn't offer embeddings)
...
class ChatbotUI {
    ...
    // ─── Multi-LLM Call: Qwen (Primary) → Gemini (Fallback) ───
    async callLLM(prompt) {
        // Build conversation history for context
        const recentHistory = this.conversationHistory.slice(-CHATBOT_CONFIG.MAX_HISTORY);

        // ── STEP 1: Try Qwen via OpenRouter (Primary — Free) ──
        const openRouterKey = CHATBOT_CONFIG.OPENROUTER_API_KEY;
        if (openRouterKey && openRouterKey !== 'YOUR_OPENROUTER_API_KEY_HERE') {
            try {
                console.log('[Chatbot] Trying primary model: Qwen 2.5 14B (OpenRouter)...');
                const text = await this._callOpenRouter(prompt, recentHistory);

                // Store in history
                this._pushHistory(prompt, text);
                return text;
            } catch (err) {
                console.warn('[Chatbot] Qwen model failed, falling back to Gemini:', err.message);
            }
        } else {
            console.log('[Chatbot] No OpenRouter API key set, using Gemini directly.');
        }

        // ── STEP 2: Fallback to Gemini models ──
        const geminiKey = CHATBOT_CONFIG.GEMINI_API_KEY;
        if (!geminiKey) throw new Error('No API keys configured. Please set your API keys.');
        ...
        for (const model of CHATBOT_CONFIG.GEMINI_MODELS) {
            try {
                ...
                const text = await this._callGeminiModelWithRetry(model, geminiBody, geminiKey);
                ...
                throw err; // Non-quota error — don't cascade
            }
        }
    ...
    // ─── OpenRouter API Call (Qwen 2.5 14B — via Proxy) ───
    async _callOpenRouter(prompt, history) {
        ...
        for (let attempt = 0; attempt < CHATBOT_CONFIG.MAX_RETRIES; attempt++) {
            try {
                const response = await fetch(`${CHATBOT_CONFIG.API_PROXY_URL}/api/chat`, {
                    ...
                    body: JSON.stringify({
                        messages,
                        model: CHATBOT_CONFIG.OPENROUTER_MODEL
                    })
                });

                if (response.status === 429) {
                    if (attempt < CHATBOT_CONFIG.MAX_RETRIES - 1) {
                        const delay = CHATBOT_CONFIG.RETRY_BASE_DELAY * Math.pow(2, attempt);
                        console.warn(`[OpenRouter] Rate limited, retry ${attempt + 1} in ${delay}ms`);
                        await this._sleep(delay);
                        continue;
                    }
                    throw new Error('429 rate limit exceeded for Qwen model');
                }

                if (!response.ok) {
                    const errData = await response.json().catch(() => ({}));
                    throw new Error(errData?.error?.message || `Proxy error: ${response.status}`);
                }

                const data = await response.json();

                // Proxy returns Gemini-style format from the worker
                const text = data?.candidates?.[0]?.content?.parts?.[0]?.text
                    || data?.choices?.[0]?.message?.content;
                if (!text) throw new Error('Empty response from Qwen model.');

                console.log('[Chatbot] ✅ Response from Qwen 2.5 14B (OpenRouter)');
                return text;
    ...
    async _callGeminiModelWithRetry(model, body, apiKey) {
            ...
            try {
                ...
                if (response.status === 429) {
                    ...
                    if (attempt < maxRetries - 1 && waitSec && waitSec < 30) {
                        console.warn(`[Gemini] Rate limited on ${model}, waiting ${waitSec}s...`);
                    ...
                    if (attempt < maxRetries - 1) {
                        const delay = CHATBOT_CONFIG.RETRY_BASE_DELAY * Math.pow(2, attempt);
                        console.warn(`[Gemini] Retry ${attempt + 1}/${maxRetries} for ${model} in ${delay}ms`);
                        await this._sleep(delay);
                        continue;
                    }
